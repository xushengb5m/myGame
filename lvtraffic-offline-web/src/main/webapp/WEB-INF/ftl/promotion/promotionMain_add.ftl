	...
	<div class="content content1">
		...
		<div class="infor1" id="conditionDiv" >
			...
			<div class="order message">
				<div class="main">
					<center>
						<h1 style="display:block">新增活动信息</h1>
					</center>
					...
					<div class="part noshow">
							<table  style="width:60%;">
								<tr>
									<#if (promotionMain.id)!''!=''>
										<td>活动ID:<input type="text" id="id" value="${(promotionMain.id)!''}"></td>
									</#if>
									...
									<td>活动有效性:
										<select id="validity" style="width:20%" >
											<#list allValids as valid>
												<#if valid != 'NONE'>
													<#if (promotionMain?? && promotionMain.validity == 'valid') || valid == 'INVALID' >
						...
						<div style="position: relative;margin-left: -7%;" id="time_and_other">
						...
						<#if (promotionMain.id)!''!=''>
							<#else>
							<div class="part" style="position: relative;margin-top: 1%;left: 40%;">
								<a href="javascript:;" onclick="insertProMain();"><div class="button">${(promotionMain.id)!''!=""}保存</div></a>
							</div>
						</#if>
					...
					<div class="click" style="position: relative;right: 21%;">
						<a href="${request.contextPath}/promotionMain/toPromotionMainList" ><div class="button">返回</div></a>
					</div>
...
<script>
	...
	function insertProMain(){
		...
		$.ajax({
			url : '${request.contextPath}/promotionMain/insertPromotionMain',
			type : 'POST',
			data : {
			...
			success : function(data){
				if(data.flag == 'SUCCESS'){
					alert("新增主活动成功！");
					window.location.href = "${request.contextPath}/promotionMain/aqueryById/"+data.mainId;
				}else{
					alert("新增主活动失败");
				}
			}
	...
	$("#add_timeSegment").click(function(){
		...
		var aa={dateFmt:'yyyy-MM-dd HH:mm'};
		var timeSegment = "<div><div class='part' name='timeSegment'><span>有效时间</span> &nbsp;&nbsp;&nbsp;&nbsp;"+
		"<input type='text' name='validTime_start'  onfocus='WdatePicker("+JSON.stringify(aa)+")' style='width:123px;' class='Wdate' readonly='readonly'/>"+
