  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function getInsuranceOrderParams() {
        	...
        	var insuranceStatusStr = $("#insuranceStatus").val();
        	if(insuranceStatusStr == 'all'){
        		insuranceStatusStr='';
        	}
        	...
        	var obj = {
				...
				'insuranceStatus' : insuranceStatusStr,
		...
		function initGrid() {
			$("#insuranceOrderList").jqGrid({
				...
				datatype : "json",
				mtype : "POST",
				colNames : ['订单号','票号','保单ID','保单号','险种名称','产品类型','产品类型','市场参考销售金额','供应商','用户姓名','证件号码','创建时间','投保时间','投保时间','生效时间','废保时间','订单保险状态','投保状态','操作'],
				colModel : [ {
				...
				gridComplete:function(){  //在此事件中循环为每一行添加日志、废保和查看链接
                    ...
                    for(var i=0; i<ids.length; i++){
                        ...
                        operateClick= '<a href="javascript:;" style="color:blue" onclick="operateLog('+insuranceId+')" >日志</a> <a href="#" style="color:blue" onclick="hesitateCancel('+"'"+insuranceNo+"' ,'"+ insureDate +"' ,'"+ productTypes +"'"+')" >废保</a> <a href="#" style="color:blue" onclick="checkDetail('+insuranceId+')" >查看</a>';
			...
			<div class="visitor message">
				...
				<div class="main">
					<div class="part">
						...
						<span>险种名称：</span><select name="insuranceInfoDto.id" id="insuranceName">
						</select>
