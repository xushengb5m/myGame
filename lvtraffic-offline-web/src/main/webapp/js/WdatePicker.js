...
function SelectDateById(id,strFormat,x,y,status)
{
 ...
 else{SelectDate(obj,strFormat,x,y,status)}
}
//选择日期 → 由 www.ttkc.net 2006-06-25 添加
function SelectDate(obj,strFormat,x,y,status)
{
...
String.prototype.toDate = function(style) {
  ...
  var dt ;
  if(xstatus=='1'){
	  eval ("dt = new Date('"+ y+"', '"+(M-1)+"','"+ d+"','00','00','00')");
  }else{
	  eval ("dt = new Date('"+ y+"', '"+(M-1)+"','"+ d+"','23','59','59')");
  }  
  return dt;
}
...
function InitContainerPanel() //初始化容器
{
 var str = '<div id="calendarPanel" style="margin-left: 25px;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>';
 if(document.all)
 {
  str += '<iframe style="position:absolute;z-index:2000;width:expression(this.previousSibling.offsetWidth);';
  str += 'height:expression(this.previousSibling.offsetHeight);';
